...
@Component({
  selector: 'app-vendor-credit-memo',
  templateUrl: './vendor-credit-memo.component.html',
  styleUrls: ['./vendor-credit-memo.component.css']
})
export class VendorCreditMemoComponent implements OnInit {
  ItemsArray= [];
  submitted=false;
  datesubmitted = false;
  ...
  redirect: 'follow'
   ...
   this.vendorcreditmemo(requestOptions).then((response)=>{
     response.json().then((result)=>{
      ...
      for(var i = 0;i < this.ItemsArray.length;i++){
         ...
         var formatteddate = this.ItemsArray[i].DOCUMENT_DATE._text.getFullYear()+'-'+('0'+(this.ItemsArray[i].DOC_DATE._text.getMonth()+1)).slice(-2)+'-'+('0'+this.ItemsArray[i].DOCUMENT_DATE._text.getDate()).slice(-2);
  ...
  vendorcreditmemo(requestOptions){
    return fetch("http://localhost:8000/vendorcreditmemo",requestOptions)
}
