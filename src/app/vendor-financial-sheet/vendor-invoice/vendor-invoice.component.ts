...
export class VendorInvoiceComponent implements OnInit {
  ...
  redirect: 'follow'
   ...
   this.vendorinvoice(requestOptions).then((response)=>{
     response.json().then((result)=>{
      ...
      for(var i = 0;i < this.ItemsArray.length;i++){
         ...
         var formatteddate = this.ItemsArray[i].DOC_DATE._text.getFullYear()+'-'+('0'+(this.ItemsArray[i].DOC_DATE._text.getMonth()+1)).slice(-2)+'-'+('0'+this.ItemsArray[i].DOC_DATE._text.getDate()).slice(-2);
         this.ItemsArray[i].DOC_DATE._text = formatteddate;
         var year = this.ItemsArray[i].POSTING_DATE._text.substring(0,4);
         var month = this.ItemsArray[i].POSTING_DATE._text.substring(4,6);
         var day = this.ItemsArray[i].POSTING_DATE._text.substring(6,8);
         this.ItemsArray[i].POSTING_DATE._text = new Date(year,month-1,day);
         var formatteddate = this.ItemsArray[i].POSTING_DATE._text.getFullYear()+'-'+('0'+(this.ItemsArray[i].POSTING_DATE._text.getMonth()+1)).slice(-2)+'-'+('0'+this.ItemsArray[i].POSTING_DATE._text.getDate()).slice(-2);
         this.ItemsArray[i].POSTING_DATE._text = formatteddate;
         console.log(this.ItemsArray[i].DOC_DATE._text,this.ItemsArray[i].POSTING_DATE._text);
      }
  ...
  vendorinvoice(requestOptions){
    return fetch("http://localhost:8000/vendorinvoice",requestOptions)
}
